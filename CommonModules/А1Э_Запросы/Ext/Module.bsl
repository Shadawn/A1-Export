...
//  ТекстЗапроса - Строка - 
//  СтрокаПоиска - Строка - 
//  Условие		 - Строка,Массив - 
// 
// Возвращаемое значение:
//   - 
//
Функция ПодставитьУсловие(ТекстЗапроса, СтрокаПоиска, Знач Условие) Экспорт
	Условие = А1Э_Массивы.Массив(Условие);
...
Функция ПодставитьЗначение(ТекстЗапроса, СтрокаПоиска, Значение) Экспорт
	Возврат А1Э_Строки.Подставить(ТекстЗапроса, СтрокаПоиска, СтрокаЗначения(Значение));	
КонецФункции
...
Функция ДополнениеНулямиСлева(ИмяПоля, ИтоговаяДлина) Экспорт 
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить("ВЫБОР");
	Для Сч = 1 ПО ИтоговаяДлина - 1 Цикл
		Текст = "КОГДА ВЫРАЗИТЬ(&ИмяПоля КАК СТРОКА(&Сч)) = &ИмяПоля ТОГДА &Нули + &ИмяПоля";
		А1Э_Строки.Подставить(Текст, "&Сч", А1Э_Строки.ВСтроку(Сч));
		А1Э_Запросы.ПодставитьСтроку(Текст, "&Нули", А1Э_Строки.Умножить("0", ИтоговаяДлина - Сч));
		МассивСтрок.Добавить(Текст);
	КонецЦикла;
	МассивСтрок.Добавить("ИНАЧЕ &ИмяПоля КОНЕЦ");
	Результат = СтрСоединить(МассивСтрок, Символы.ПС);
	А1Э_Строки.Подставить(Результат, "&ИмяПоля", ИмяПоля);
	Возврат Результат;
КонецФункции
