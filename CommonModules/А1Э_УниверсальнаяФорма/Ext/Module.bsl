...
Функция ИнициализироватьФормуПриНеобходимости(Форма) Экспорт 
	...
	#Иначе
		...
		А1Э_Формы.ДобавитьОписаниеРеквизита(МассивОписаний, "А1_УниверсальнаяФорма__СерверныеОбработчики", "");
		...
		Форма.А1_УниверсальнаяФорма__СерверныеОбработчики = А1Э_Структуры.Создать(
...
// Создает строку для выполнения обработчика асинхронно. Исправлены коллизии имен.
//
// Параметры:
//  ИмяФункции	 - 	 - 
//  П1			 - 	 - 
...
//   - 
//
Функция СтрокаВыполненияОбработчикаАсинхронно1_1_3(Знач ИмяФункции, П1 = Null, П2 = Null, П3 = Null, П4 = Null, П5 = Null) Экспорт 
	СтрокаВыполнения = СтрокаВыполненияОбработчика1_1_1(ИмяФункции, П1, П2, П3, П4, П5);
	Возврат СтрЗаменить(СтрокаВыполнения, "(", "(А1Э_АсинхронныйКонтекст, ");
КонецФункции
...
Функция ДобавитьДействиеКомандыФормыСлужебно(Форма, Команда, Обработчик, Индекс = Неопределено, Замещать = Ложь);  
	...
	Если НЕ Форма.А1_УниверсальнаяФорма__ОбработчикиКоманд.Свойство(ИмяКоманды) Тогда
		...
		НеобходимаПривязка = Истина;
	Иначе
		КоллекцияДействий = Форма.А1_УниверсальнаяФорма__ОбработчикиКоманд[ИмяКоманды];
		Если ТипЗнч(КоллекцияДействий) <> Тип("Массив") Тогда
			Форма.А1_УниверсальнаяФорма__ОбработчикиКоманд.Вставить(ИмяКоманды, Обработчики(КоллекцияДействий));
		КонецЕсли;
		КоллекцияДействий = Форма.А1_УниверсальнаяФорма__ОбработчикиКоманд[ИмяКоманды];
	КонецЕсли;
	Если НеобходимаПривязка = Истина Тогда
...
Функция НастройкиКомпонента(Форма, ИмяКомпонента) Экспорт 
	Возврат А1Э_Общее.ЗначениеСвойства(Форма, ИмяНастроекКомпонента(ИмяКомпонента)); 
КонецФункции
...
#Если Клиент Тогда
	...
	Функция ЗаписатьЕслиНовый(АсинхронныйКонтекст, Форма, Команда) Экспорт 
		А1Э_Формы.ЗаписатьСПредупреждениемПриНеобходимости(Форма, "ЗавершитьАсинхронныйОбработчикКоманды", А1Э_Структуры.Создать(
		"Модуль", ЭтотОбъект,
		"АсинхронныйКонтейнер",  АсинхронныйКонтейнер(АсинхронныйКонтекст, Форма, Команда),
		"ВариантПроверки", "Новый",
		));
	КонецФункции
