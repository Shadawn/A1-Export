#Если Клиент Тогда
	...
	//   - Неопределено
	//
	Функция Открыть(Заголовок, ИмяФункцииПриСозданииНаСервере, Знач Параметры = Неопределено) Экспорт
		...
		ОткрытьФорму("ОбщаяФорма.А1Э_УниверсальнаяФорма", РабочиеПараметры, , Истина, , , , РежимОткрытияОкнаФормы.Независимый);
	КонецФункции
...
#Если НЕ Клиент Тогда
	...
	//   - 
	//
	Функция __ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт  
	...
	Функция ДобавитьДействияУниверсальнойФормы(Форма, ЭлементФормы, ОписаниеЭлементаФормы) 
		...
		Для Каждого Действие Из ОписаниеЭлементаФормы.Действия Цикл
			Если НЕ ЭтоСобытиеФормы(Действие.Ключ) Тогда
...
Функция ВыполнитьОбработчикиСобытияДляОбъектовФормы(ИмяОбработчика, Форма, П1 = Null, П2 = Null, П3 = Null, П4 = Null, П5 = Null, П6 = Null, П7 = Null) Экспорт
	...
	Для Каждого Пара Из Форма.А1_УниверсальнаяФорма__ОбработчикиЭлементов Цикл
		ИмяОбъектаФормы = Пара.Ключ;
		ВсеОбработчикиОбъектаФормы = Пара.Значение;
		
		Если НЕ ВсеОбработчикиОбъектаФормы.Свойство(ИмяОбработчика) Тогда Продолжить; КонецЕсли;
...
// Добавляет обработчик к массиву действий
//
// Параметры:
//  Действия	 - Массив -  
...
// Добавляет обработчик из модуля к массиву действий. Предполагает, что имя действия равно имени процедуры. 
//
// Параметры:
//  Действия	 - Массив - 
...
Функция ПроверитьДействие(Действия, Знач ИмяДействия) 
	...
	КонецЕсли;
	Если ЭтоСобытиеФормы(ИмяДействия) Тогда
		НеобходимаПривязка = Ложь;
	КонецЕсли;
...
Функция НастройкиКомпонента(Форма, ИмяКомпонента) Экспорт 
	Возврат Форма[ИмяКомпонента + "___Настройки"]; 
КонецФункции

Функция ДобавитьОписаниеНастроекКомпонента(МассивОписаний, ИмяКомпонента, ЗначениеНастроек) Экспорт
	Возврат А1Э_Формы.ДобавитьОписаниеРеквизита(МассивОписаний, ИмяКомпонента + "___Настройки", "", , ,
...
#Если Клиент Тогда
	...
	Функция ЗаписатьПриНеобходимости(АсинхронныйКонтекст, Форма, Команда) Экспорт
		А1Э_Формы.ЗаписатьСПредупреждениемПриНеобходимости(Форма, "ЗаписатьПриНеобходимостиЗавершение", А1Э_Структуры.Создать(
		"Модуль", ЭтотОбъект,
		"АсинхронныйКонтейнер",  АсинхронныйКонтейнер(АсинхронныйКонтекст, Форма, Команда),
		));
	КонецФункции
	
	Функция ЗаписатьПриНеобходимостиЗавершение(Результат, Контекст) Экспорт
