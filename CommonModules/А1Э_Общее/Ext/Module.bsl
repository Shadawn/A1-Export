...
Функция ЗначениеСвойства(Источник, ИмяСвойства, ЗначениеПоУмолчанию = Неопределено) Экспорт
	...
	Если ИмяСвойства = "" Тогда Возврат ЗначениеПоУмолчанию; КонецЕсли;
...
// Аналогично ЗаполнитьЗначенияСвойств(), но с изменениями поведения в крайних случаях
//  1. Если исключаемого свойства нет в свойства источника, то этот параметр игнорируется
//
// Параметры:
//  Приемник		 - Произвольный	 - 
//  Источник		 - Произвольный	 - 
//  СписокСвойств	 - Строка,Массив<Строка> - 
//  ИсключаяСвойства - Строка,Массив<Строка> - 
// 
// Возвращаемое значение:
//   - 
//
Функция ПеренестиСвойства(Приемник, Источник, Знач СписокСвойств = Неопределено, Знач ИсключаяСвойства = Неопределено) Экспорт
	Если Источник = Неопределено Тогда Возврат Неопределено; КонецЕсли;
	
	ИсключаяСвойства = А1Э_Массивы.Массив(ИсключаяСвойства);
	РабочиеИсключаемыеСвойстваМассив = Новый Массив;
	Для Каждого ИсключаемоеСвойство Из ИсключаяСвойства Цикл
		Если НЕ Свойство(Источник, ИсключаемоеСвойство) Тогда Продолжить; КонецЕсли;
		РабочиеИсключаемыеСвойстваМассив.Добавить(ИсключаемоеСвойство);
	КонецЦикла;
	ЗаполнитьЗначенияСвойств(Приемник, Источник, СписокСвойств, СтрСоединить(РабочиеИсключаемыеСвойстваМассив, ","));
КонецФункции
