...
#Если Клиент Тогда
	...
	//  ИмяФункцииДанныхПечати	 - Строка - полное имя серверной функции, которая принимает ОбъектыПечати и возвращает данные печати. 
	...
	//   - 
	//
	Функция Напечатать2(ИмяМакета, ИмяФункцииДанныхПечати, ОбъектыПечати, ИмяРезультата = "Документ Word", Архивировать = Ложь, ДополнительныеПараметры = Неопределено) Экспорт
		РабочиеПараметры = А1Э_Структуры.СкопироватьВШаблонСЗаменой(ДополнительныеПараметры,
		"РазделительЛевый", "{",
		"РазделительПравый", "}",
		);
		Напечатать(ИмяМакета, ИмяФункцииДанныхПечати, ОбъектыПечати, ИмяРезультата, Архивировать, РабочиеПараметры);		
	КонецФункции 
...
#Если НЕ Клиент Тогда
	...
	Функция ВывестиДанныеЧерезХМЛСервер(ПутьКМакету, ВнешниеДанныеЗамены, Архивировать = Ложь, ДополнительныеПараметры = Неопределено) Экспорт
		МассивДанныхЗамены = А1Э_Массивы.Массив(ВнешниеДанныеЗамены);
		ДанныеЗамены = Новый Массив;
		Сч = -1;
		Пока Сч < МассивДанныхЗамены.Количество()-1 Цикл
			Сч = Сч + 1;
			ЭлементДанныхЗамены = МассивДанныхЗамены[Сч];
			СтруктураЗамены = Новый Структура("ИмяФайла,ДанныеЗамены");
			...
			ИначеЕсли ТипЗнч(МассивДанныхЗамены[Сч]) = Тип("Соответствие") Тогда
	...
	Функция СоздатьАрхивФайловВорд(ПутьКМакету, ПараметрыЗаполненияШаблона, ДанныеЗамены)
		...
		Для Каждого Элемент Из ДанныеЗамены Цикл
			...
			МодульУправлениеПечатью.МакетПечатнойФормы(ПутьКМакету(Элемент, ПутьКМакету)).Записать(ВременныйФайл);
			ДанныеФайла = РаспаковатьШаблонDocx(ВременныйФайл);
	...
	Функция СоздатьЕдиныйФайлВорд(ПутьКМакету, ПараметрыЗаполненияШаблона, ДанныеЗамены)
		...
		МодульУправлениеПечатью = Вычислить("УправлениеПечатью");
		МодульУправлениеПечатью.МакетПечатнойФормы(ПутьКМакету(ДанныеЗамены[0], ПутьКМакету)).Записать(ФайлШаблона);
...
#Если Клиент Тогда
	...
	// Параметры:
	//  ИмяМакета				 - Строка - полное имя макета (включая путь). 
	//  ИмяФункцииДанныхПечати	 - Строка - полное имя серверной функции, которая принимает массив объектов и возвращает данные печати 
	//  ОбъектыПечати           - Массив - массив ссылок на объекты печати
	//  ИмяРезультата			 - Строка - имя файла, который будет передан пользователю. 
	//  Архивировать			 - Булево - если Истина, то результат будет выдан в качестве скачиваемого архива. Если Ложь, то результат будет показан пользователю файлом.
	//  ДополнительныеПараметры - Структура - Может содержать ключи "РазделительЛевый", "РазделительПравый" (по умолчанию "<", ">") 
	...
	//   - 
	//
	Функция Напечатать(ИмяМакета, ИмяФункцииДанныхПечати, ОбъектыПечати, ИмяРезультата = "Документ Word", Архивировать = Ложь, ДополнительныеПараметры = Неопределено) Экспорт
		ДанныеЗамены = А1Э_ОбщееСервер.РезультатФункции(ИмяФункцииДанныхПечати, ОбъектыПечати); 
