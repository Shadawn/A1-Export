...
#Если НЕ Клиент Тогда
	...
	// Создает иерархическую структуру, аналогичную по логике работы с типовым ДеревомЗначений
	//
	// Параметры:
	...
	//   - 
	//
	Функция ИзДереваЗначений(ДеревоЗначений) Экспорт
		Если ДеревоЗначений.Колонки.Найти("Строки") <> Неопределено Тогда
			А1Э_Служебный.СлужебноеИсключение("ДеревоЗначений с колонкой <Строки> невозможно преобразовать в иерархию!");
		КонецЕсли;
		Строки = Новый Массив;
		Для Каждого СтрокаДерева Из ДеревоЗначений.Строки Цикл
			ДобавитьСтрокиВСтруктуруСПодчиненными(Строки, СтрокаДерева, ДеревоЗначений);
		КонецЦикла;
		Возврат А1Э_Структуры.Создать(
		"Класс", "А1Э_ДеревоЗначенийСтруктура",
		"Колонки", КолонкиДереваЗначений(ДеревоЗначений),
		"Строки", Строки,
		);
	КонецФункции 
	
	Функция КолонкиДереваЗначений(ДеревоЗначений) 
		Колонки = Новый Массив;
		Для Каждого Колонка Из ДеревоЗначений.Колонки Цикл
			СтруктураКолонки = Новый Структура("Имя,ТипЗначения,Заголовок,Щирина");
			ЗаполнитьЗначенияСвойств(СтруктураКолонки, Колонка);
			Колонки.Добавить(СтруктураКолонки);
		КонецЦикла;
		Возврат Колонки;
	КонецФункции
	
	Функция ДобавитьСтрокиВСтруктуруСПодчиненными(Строки, СтрокаДерева, ДеревоЗначений) 
		Строка = Новый Структура("Строки", Новый Массив);
		Строки.Добавить(Строка);
		Для Каждого Колонка Из ДеревоЗначений.Колонки Цикл
			Строка.Вставить(Колонка.Имя, СтрокаДерева[Колонка.Имя]);
		КонецЦикла;
		Для Каждого ПодчиненнаяСтрокаДерева Из СтрокаДерева.Строки Цикл
			ДобавитьСтрокиВСтруктуруСПодчиненными(Строка.Строки, ПодчиненнаяСтрокаДерева, ДеревоЗначений); 
		КонецЦикла;
	КонецФункции 
