...
#Если Не Клиент Тогда
	...
	Функция СоздатьЭлементПоОписанию(Форма, ОписаниеРеквизита) Экспорт
		...
		Если ТипЗнч(ЭлементФормы) = Тип("ГруппаФормы") И ОписаниеРеквизита.Параметры.Свойство("Группировка") И А1Э_Общее.Свойство(ЭлементФормы, "Группировка") Тогда
			ГруппировкаВПараметрах = ОписаниеРеквизита.Параметры.Группировка;
			Если ТипЗнч(ГруппировкаВПараметрах) = Тип("Строка") Тогда
				ГруппировкаСтрока = ГруппировкаВПараметрах;
			Иначе
				ГруппировкаСтрока = Строка(ГруппировкаВПараметрах);
			КонецЕсли;
		...
		ЗаполнитьЗначенияСвойств(ЭлементФормы, ОписаниеРеквизита.Параметры, , Исключения);
...
#Если НЕ Клиент Тогда
	...
	//	
	//	Возврат ДобавитьОписаниеЭлемента(МассивОписаний, Имя, Контекст.Путь, А1Э_Структуры.ЗначениеСвойства(КонтекстКолонки, "Заголовок"), Контекст.Родитель, , КонтекстКолонки, А1Э_Структуры.ЗначениеСвойства(КонтекстКолонки, "Действия"));  
	//КонецФункции
