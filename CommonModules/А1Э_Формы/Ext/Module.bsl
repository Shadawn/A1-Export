...
#Если Не Клиент Тогда
	...
	Функция ДобавитьОписаниеСтраницы(МассивОписаний, Имя, Заголовок = "", РодительЭлемента, ПередЭлементом = Неопределено, Параметры = Неопределено) Экспорт 
		РабочиеПараметры = А1Э_Структуры.СкопироватьССозданием(Параметры);
		РабочиеПараметры.Вставить("Тип", Тип("ГруппаФормы"));
		А1Э_Структуры.ВставитьЕслиОтсутствует(РабочиеПараметры, "Вид", ВидГруппыФормы.Страница);
		
		Возврат ДобавитьОписаниеЭлемента(МассивОписаний, Имя, , Заголовок, РодительЭлемента, ПередЭлементом, РабочиеПараметры);
	КонецФункции 
	...
	Функция СоздатьЭлементПоОписанию(Форма, ОписаниеРеквизита) Экспорт
		...
		ЭлементФормы = Форма.Элементы.Вставить(ИмяЭлемента, ОписаниеРеквизита.Параметры.Тип, ЭлементФормы(Форма, ОписаниеРеквизита.РодительЭлемента), ЭлементФормы(Форма, ОписаниеРеквизита.ПередЭлементом));
		Если НЕ ОписаниеРеквизита.СоздаватьРеквизит Тогда
		...
		Если Тип(ЭлементФормы) = Тип("ПолеФормы") Тогда
			...
			Попытка //Разными способами проверяем булевые реквизиты. В частности ловим ситуацию когда создается только элемент и тип реквизита не указывается.
				Если ТипЗнч(Реквизит(Форма, ОписаниеРеквизита.Имя, ОписаниеРеквизита.ПутьКРеквизиту)) = Тип("Булево")
	...
	Функция ИмяТаблицыПоляФормы(Форма, Знач РодительЭлемента)
		Если ТипЗнч(РодительЭлемента) = Тип("Строка") Тогда
...
#Если НЕ Клиент Тогда
	...
	Функция ДобавитьОписаниеГоризонтальнойГруппы(МассивОписаний, Имя, Заголовок = "", РодительЭлемента = Неопределено, ПередЭлементом = Неопределено, Параметры = Неопределено) Экспорт
		...
		А1Э_Формы.ДобавитьОписаниеГруппы(МассивОписаний, Имя, Заголовок, РодительЭлемента, ПередЭлементом, РабочиеПараметры);
	КонецФункции
	
	Функция ДобавитьОписаниеВертикальнойГруппы(МассивОписаний, Имя, Заголовок = "", РодительЭлемента = Неопределено, ПередЭлементом = Неопределено, Параметры = Неопределено) Экспорт
		РабочиеПараметры = А1Э_Структуры.СкопироватьВШаблон(Параметры,
	...
	Функция ДобавитьОписаниеТаблицыОбъекта(Форма, МассивОписаний, Имя, Знач Колонки = Неопределено, Заголовок = "", РодительЭлемента = Неопределено, ПередЭлементом = Неопределено, Параметры = Неопределено, Действия = Неопределено) Экспорт  
		...
		Если НЕ РабочиеКолонки.Свойство("НомерСтроки") Тогда
			РабочиеКолонки.Вставить(А1Э_Структуры.Создать("Ширина", 2));
		КонецЕсли;
		...
		ДобавитьОписаниеКолонкиТаблицыОбъекта(МассивОписаний, "НомерСтроки", Контекст); 
		
		Для Каждого Реквизит Из МетаданныеТаблицы.Реквизиты Цикл
			ДобавитьОписаниеКолонкиТаблицыОбъекта(МассивОписаний, Реквизит.Имя, Контекст); 
		КонецЦикла;
...
//
	...
	//  Имя				 - Строка	 - 
	//  ПараметрыСписка	 - Структура - ключи: ТекстЗапроса(Строка), ПараметрыЗапроса(Структура), ДинамическоеСчитываниеДанных(Булево)
	//  Заголовок		 - Строка	 - 
	//  РодительЭлемента - Строка, ЭлементФормы	 - 
	//  ПередЭлементом	 - Строка, ЭлементФормы	 - 
	//  Параметры		 - Структура - 
...
#Если НЕ Клиент Тогда
	...
	Функция ДобавитьОписаниеШапкиСДвумяКолонками(МассивОписаний, Имя = "ГруппаШапка", РодительЭлемента = Неопределено, ПередЭлементом = Неопределено) Экспорт
		А1Э_Формы.ДобавитьОписаниеГоризонтальнойГруппы(МассивОписаний, Имя, , РодительЭлемента, ПередЭлементом);
...
Функция ТаблицаЭлемента(Форма, Элемент) Экспорт
	Если ТипЗнч(Элемент.Родитель) = Тип("ТаблицаФормы") Тогда Возврат Элемент.Родитель; КонецЕсли;
	Возврат ТаблицаЭлемента(Форма, Элемент.Родитель);
КонецФункции
...
#Если НЕ Клиент Тогда
	...
	Функция ДобавитьОписаниеКолонкиТаблицыОбъекта(МассивОписаний, Имя, Контекст)  
		КонтекстКолонки = А1Э_Структуры.ЗначениеСвойства(Контекст.Колонки, Имя); 
		Если КонтекстКолонки = Неопределено И Контекст.ТолькоУказанныеКолонки = Истина Тогда Возврат Неопределено КонецЕсли;
		Если А1Э_Структуры.Свойство(КонтекстКолонки, "НеСоздавать") Тогда Возврат Неопределено КонецЕсли;
		
		Возврат ДобавитьОписаниеЭлемента(МассивОписаний, Имя, Контекст.Путь, А1Э_Структуры.ЗначениеСвойства(КонтекстКолонки, "Заголовок"), Контекст.Родитель, , А1Э_Структуры.ЗначениеСвойства(КонтекстКолонки, "Параметры"), А1Э_Структуры.ЗначениеСвойства(КонтекстКолонки, "Действия"));  
	КонецФункции
	
#КонецЕсли

	Функция ДанныеКолонкиТаблицыФормы(ИсточникДанных) Экспорт
		ДанныеКолонки = Новый Структура("Имя,Тип,Заголовок,Параметры,Действия");
...
#Если Клиент Тогда
	...
	Функция ПоказатьВыбор(Форма, ИмяОбъекта = Неопределено, ИмяПроцедурыДальнейшихДействий, ПараметрыОткрытия = Неопределено) Экспорт
		ВнешнееОповещение = А1Э_СтандартныеТипы.ОписаниеОповещенияПолучить(ИмяПроцедурыДальнейшихДействий, Форма);
	...
	Процедура ОткрытьУниверсальнуюФормуВыбора(Форма, ИмяОбъекта = Неопределено, ОповещениеОВыборе, ПараметрыОткрытия)
		Параметры = Новый Структура;
		Параметры.Вставить("А1_ПриСозданииНаСервере", "А1Э_Формы.ПоказФормыВыбораЧерезУниверсальнуюФорму_ПриСозданииНаСервере");
		Параметры.Вставить("А1_ЗаголовокФормы", А1Э_Структуры.ЗначениеСвойства(ПараметрыОткрытия, "Заголовок", "Выберите объект"));
		
		Параметры.Вставить("ИмяОбъекта", ИмяОбъекта);
		Параметры.Вставить("ПараметрыОткрытия", ПараметрыОткрытия);
		ОткрытьФорму("ОбщаяФорма.А1Э_УниверсальнаяФорма", Параметры, Форма, , , , ОповещениеОВыборе, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца); 
	КонецПроцедуры
