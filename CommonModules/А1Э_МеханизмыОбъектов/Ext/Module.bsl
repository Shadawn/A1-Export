...
#Область ПроверкаУникальностиРеквизита
...
#Если НЕ Клиент Тогда
	Функция ПроверкаУникальностиРеквизитов__ПередЗаписью(Объект, Отказ, РежимЗаписи = Неопределено, РежимПроведения = Неопределено) Экспорт
		...
		Если ТипЗнч(Контекст) <> Тип("Массив") Тогда
			А1Э_Служебный.СлужебноеИсключение("Неверный тип контекста в механизме А1Э_ПроверкаУникальностиРеквизита");
		КонецЕсли;
...
Функция СписокИзменений__ПередЗаписью(Объект, Отказ, РежимЗаписи = Неопределено, РежимПроведения = Неопределено) Экспорт
	Объект.ДополнительныеСвойства.Вставить("А1Э_СписокИзменений", СписокИзменений(Объект));
КонецФункции

Функция СписокИзменений(Объект) Экспорт
	Изменения = Новый Структура;
