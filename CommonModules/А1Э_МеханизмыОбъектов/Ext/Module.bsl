...
#Если НЕ Клиент Тогда
	...
	Функция СохранениеИдентификаторов__ПередЗаписью(Объект, Отказ, РежимЗаписи = Неопределено, РежимПроведения = Неопределено) Экспорт
		...
		ИначеЕсли ЗначениеЗаполнено(Объект.ПолучитьСсылкуНового()) Тогда
...
#Если НЕ Клиент Тогда
	Функция ПроверкаНового__ПередЗаписью(Объект, Отказ, РежимЗаписи = Неопределено, РежимПроведения = Неопределено) Экспорт 
		ТипМетаданных = А1Э_Метаданные.ТипМетаданных(Объект.Метаданные());
		Если А1Э_Общее.РавноОдномуИз(ТипМетаданных, "РегистрСведений", "РегистрНакопления", "РегистрБухгалтерии", "РегистрРасчета") Тогда Возврат Неопределено; КонецЕсли;
...
#Если НЕ Клиент Тогда
	...
	Функция Нумерация__УстановитьНовыйНомер(Объект, МетаданныеОбъекта, ИмяПоля, ДлинаПоля)
		...
		Если МаксимальныйНомер = Неопределено Тогда
			Объект[ИмяПоля] = А1Э_Строки.Умножить("0", ДлинаПоля - 1) + "1";
		Иначе
			Объект[ИмяПоля] = А1Э_Строки.ДополнитьЛидирующимиНулями(А1Э_Строки.ВСтроку(Число(МаксимальныйНомер) + 1), ДлинаПоля);
		КонецЕсли;
