	...
	Функция ВыполнитьКоманду(ИмяКоманды, КонтекстКоманды = Неопределено, КонтекстВыполнения = Неопределено) Экспорт
		Если КонтекстВыполнения = Неопределено Тогда
			Если А1Э_Пользователи.ОткрытКонфигуратор() Тогда
				...
				Возврат Ложь;
			КонецЕсли;
			ПараметрыКоманды = "DESIGNER /DisableStartupMessages /DisableStartupDialogs /" + ИмяКоманды;
			Для Каждого Элемент Из КонтекстКоманды Цикл
				А1Э_Строки.Добавить(ПараметрыКоманды, Элемент, " ");
			КонецЦикла;
		
			ЗапуститьСистему(ПараметрыКоманды, Истина);
		...
		Если А1Э_Структуры.ЗначениеСвойства(РабочийКонтекстВыполнения.Контекст, "ПоказатьСкрипт") = Истина Тогда
			Сообщить(ТекстКоманды(ИмяКоманды, РабочийКонтекстВыполнения, КонтекстКоманды));
			Возврат Неопределено;
		КонецЕсли;
				
		А1Э_ОбщееКлиент.ВыполнитьСкрипт(ТекстКоманды(ИмяКоманды, РабочийКонтекстВыполнения, КонтекстКоманды));
	КонецФункции
