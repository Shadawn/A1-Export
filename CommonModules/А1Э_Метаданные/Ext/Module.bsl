#Если НЕ Клиент Тогда
	...
	Функция МенеджерТипаОбъектов(ТипОбъекта) Экспорт
		Идентификатор = Лев(ВРЕГ(ТипОбъекта), 3);
		...
		ИначеЕсли Идентификатор = "РЕГ" Тогда
			ДопИдентификатор = ВРЕГ(Сред(ТипОбъекта, 8, 3));
	...
	//
	// Параметры:
	//  Идентификатор	 - Строка, Ссылка, Тип - если строка, то можно указывать полное имя (Справочник.Номенклатура) или сокращенное (Номенклатура).
	...
	Функция МенеджерОбъекта(Идентификатор, Знач ТипОбъекта = Неопределено) Экспорт
		...
		Иначе
			А1Э_Служебный.ИсключениеНеверныйТип("Идентификатор", "А1Э_Метаданные.МенеджерОбъекта", Идентификатор, "Строка,Тип,Ссылка");
		КонецЕсли;
	КонецФункции 
	...
	Функция МенеджерПоСсылке(Ссылка) Экспорт 
		...
		//Иначе
		//	А1Э_Служебный.ИсключениеНеверныйТип("Ссылка", "А1Э_Метаданные.МенеджерПоСсылке", Ссылка, "Ссылка");
		//КонецЕсли;
	...
	Функция ОбъектМетаданных(Идентификатор) Экспорт
		...
		КонецЕсли;
		//Для получения метаданных по ссылке использовать Ссылка.Метаданные()!
	...
	Функция ТипМетаданныхПоИмени(Имя) Экспорт 
		Идентификатор = ВРЕГ(Лев(Имя, 3));
		Если Идентификатор = "СПР" Тогда
			Возврат Метаданные.Справочники;
		ИначеЕсли Идентификатор = "ДОК" Тогда
			Возврат Метаданные.Документы;
		ИначеЕсли Идентификатор = "РЕГ" Тогда
			ДопИдентификатор = ВРЕГ(Сред(Имя, 8, 3));
		...
		ИначеЕсли Идентификатор = "ОБЩ" Тогда
			ДопИдентификатор = ВРЕГ(Сред(Имя, 6, 3));
	...
	Функция ОбщийМодуль(Имя) Экспорт
		Если Метаданные.ОбщиеМодули.Найти(Имя) = Неопределено Тогда Возврат Неопределено; КонецЕсли;
...
Функция СсылкаПоИдентификатору(Идентификатор) Экспорт
	...
	#Иначе
		...
		ОписаниеМетаданных = СвязиСсылокИСистемныхКодов().ПоКоду[ОписаниеИдентификатора.Код];
	...
	Функция РассчитатьСвязиСсылокИСистемныхКодов() Экспорт
		...
		Для Каждого Коллекция Из Коллекции Цикл
			...
			Для Каждого ОбъектМетаданных Из МетаданныеКоллекции Цикл
				...
				ПоКодам.Вставить(КодОбъекта, ОписаниеОбъектаМетаданных);
				ПоСсылкам.Вставить(ПустаяСсылка, КодОбъекта);
			КонецЦикла;
		КонецЦикла;
		Результат = Новый ФиксированнаяСтруктура(А1Э_Структуры.Создать(
		"ПоКоду", Новый ФиксированноеСоответствие(ПоКодам),
		"ПоСсылке", Новый ФиксированноеСоответствие(ПоСсылкам)));
		Возврат Результат;
	КонецФункции 
