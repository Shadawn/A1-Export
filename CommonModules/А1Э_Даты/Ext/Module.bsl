...
// Возвращаемое значение:
//   Дата
Функция ИзЖСОН(СтрокаJSON) Экспорт
	
	Строка =
		Сред(СтрокаJSON, 1, 4) +
		Сред(СтрокаJSON, 6, 2) +
		Сред(СтрокаJSON, 9, 2) +
		Сред(СтрокаJSON, 12, 2) +
		Сред(СтрокаJSON, 15, 2) +
		Сред(СтрокаJSON, 18, 2);
	
	Возврат ДатаИзСтроки(Строка);
...
Функция А1Э_КонецНедели(Дата) Экспорт
	КонстантыДатыВремени = КонстантыДатыВремени();
	Возврат НачалоДня(НачалоНедели(Дата))
	+ КонстантыДатыВремени.СекундВЧасе * КонстантыДатыВремени.ЧасовВДне * КонстантыДатыВремени.ДнейВНеделе
	- 1;
КонецФункции

// Возвращает ОписаниеТипа с указанным квалификатором даты
// Параметры:
//  ЧастьДаты - Строка
// Возвращаемое значение:
//   ОписаниеТипов
Функция ОписаниеТипа(Знач ЧастьДаты = Неопределено) Экспорт
	...
	Иначе
		Если ТипЗнч(ЧастьДаты) = Тип("Строка") Тогда
			ЧастьДаты = ЧастиДаты[ЧастьДаты];
		КонецЕсли;
		Возврат Новый ОписаниеТипов("Дата", , , , , Новый КвалификаторыДаты(ЧастьДаты));
	КонецЕсли;
КонецФункции

// Обертка для функции СтрВДату()
...
Функция СтрВДату(Знач сДата)
	...
	Если п_мТЕМП[3] = ЦифрВГоду_2
		...
		Год_1950Числом = 50;
		Если п_мТЕМП[3] < Год_1950Числом Тогда
...
Процедура СтрВДату_Месяц(п_мТЕМП)
	...
	МинимальнаяДлинаПриМесяцеСтрокой = 2;
	ЧислоМесяцевВгоду = 12;
	
	Если СтрДлина(п_мТЕМП[1]) > МинимальнаяДлинаПриМесяцеСтрокой Тогда
		
		п_мТЕМП[2] = 0;
		Пока п_мТЕМП[2] < ЧислоМесяцевВгоду Цикл
...
// Возвращает Структуру констант времени
Функция КонстантыДатыВремени()
	
	СтруктураКонстант = Новый Структура;
	СтруктураКонстант.Вставить("ДнейВНеделе", 7);
	СтруктураКонстант.Вставить("ЧасовВДне", 24);
	СтруктураКонстант.Вставить("СекундВЧасе", 3600);
