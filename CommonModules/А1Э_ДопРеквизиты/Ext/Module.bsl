...
//  ЛюбоеЗначение - значение допреквизита. Если он никогда не был задан, возвращает ЗначениеПоУмолчанию. 
//
...
// Получает значения нескольких дополнительных реквизитов по их программным именам. 
//
// Параметры:
//  Ссылка	 - ЛюбаяСсылка -  
//  Имена	 - Строка,Массив - имена допреквизитов в виде строки (через запятую) или массива.
//	УбиратьПрефиксы - Булево - ожидается, что имя доп. реквизита может содержать префикс (через "__" или "_"). Если этот аргумент установлен, то префиксы будут удалены из ключей возвращаемой структуры.
//  ВидРеквизитов - Строка, Булево - вид дополнительного реквизита. Поддерживается
//		"ДР", Ложь - допреквизит, 
//		"ДС", Истина - допсведение, 
//		"А1" - данные регистра А1_Реквизиты.
//		"А1П" - данные регистра А1_ПериодическиеРеквизиты.
...
//
// Возвращаемое значение:
//  Структура<[ИмяПоля]:Произвольный> 
//
...
// Получает значения дополнительного реквизита по нескольким ссылкам. 
//
// Параметры:
//  Ссылки	 - ЛюбаяСсылка,Массив<ЛюбаяСсылка> -  
//  Имена	 - Строка,Массив<Строка> - имена допреквизитов в виде строки (через запятую) или массива.
//  ВидРеквизита - Строка, Булево - вид дополнительного реквизита. Поддерживается
//		"ДР", Ложь - допреквизит, 
//		"ДС", Истина - допсведение, 
//		"А1" - данные регистра А1_Реквизиты.
...
//
// Возвращаемое значение:
//  Соответствие<[Ссылка]:Произвольный> 
//
...
// Получает значения нескольких дополнительных реквизитов по нескольким ссылкам. 
//
// Параметры:
//  Ссылки	 - ЛюбаяСсылка,Массив<ЛюбаяСсылка> -  
//  Имена	 - Строка,Массив<Строка> - имена допреквизитов в виде строки (через запятую) или массива.
...
//
// Возвращаемое значение:
//  Соответствие<[Ссылка]:Структура<[ИмяПоля]:Произвольный>> 
//
Функция ЗначенияСсылок(Знач Ссылки, Знач Имена, УбиратьПрефиксы = Истина, Знач ВидРеквизитов = Ложь, ЗначениеПоУмолчанию = Неопределено, Контекст = Неопределено) Экспорт
	...
	Для Каждого Ссылка Из Ссылки Цикл
		...
		СоответствиеСсылкам.Вставить(Ссылка, СтруктураИмен);
		Для Каждого Имя Из Имена Цикл
