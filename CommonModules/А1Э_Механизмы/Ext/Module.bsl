...
#Если Клиент Тогда
	...
	Функция УстановитьРасширение(Объекты = Неопределено) Экспорт
		...
		ИмяФайлаРасширения = СоздатьРасширениеМеханизмовКлиент(, КаталогСоздания);
	...
	Функция СоздатьРасширениеМеханизмовКлиент(Объекты = Неопределено, КаталогСоздания = Неопределено, Контекст = Неопределено) Экспорт
		...
		А1Э_Конфигуратор.ЗагрузитьРасширениеИзФайлов(КаталогСоздания, "А1ЭМ");
		УдалитьФайлы(КаталогСоздания);
		ИмяФайлаРасширения = КаталогВременныхФайлов() + "A1MechanismExtension.cfe";
		А1Э_Конфигуратор.СохранитьКонфигурациюВФайл(ИмяФайлаРасширения, "А1ЭМ");
		А1Э_Конфигуратор.УдалитьРасширение("А1ЭМ");
