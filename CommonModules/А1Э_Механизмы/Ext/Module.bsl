...
Функция ВыполнитьМеханизмыОбработчикаОбъектов(ИмяОбработчиков, МассивОбъектов, П1 = Null, П2 = Null, П3 = Null, П4 = Null, П5 = Null, П6 = Null, П7 = Null) Экспорт
	СоответствиеОбъектов = Новый Соответствие;
	МассивИменМетаданных = Новый Массив;
	Для Каждого Объект Из МассивОбъектов Цикл
		ИмяМетаданныхОбъекта = ИмяМетаданныхОбъекта(Объект);
		Если СоответствиеОбъектов[ИмяМетаданныхОбъекта] = Неопределено Тогда
			СоответствиеОбъектов.Вставить(ИмяМетаданныхОбъекта, Новый Массив);
			МассивИменМетаданных.Добавить(ИмяМетаданныхОбъекта);
		КонецЕсли;
		СоответствиеОбъектов[ИмяМетаданныхОбъекта].Добавить(Объект);
	КонецЦикла;
	СоответствиеОбъектов.Вставить(Адресация__ВсеОбъекты(), МассивОбъектов);
	
	ФункцииОбработчиков = ФункцииОбработчиковОбъектов(ИмяОбработчиков, МассивИменМетаданных);
	
	Результат = ВыполнитьФункцииОбработчиковОбъектов(ФункцииОбработчиков, СоответствиеОбъектов, П1, П2, П3, П4, П5, П6, П7);  
	Если Результат = ПрерываниеВыполнения() Тогда Возврат Результат; КонецЕсли;
...
Функция ВыполнитьФункцииОбработчиковОбъектов(ФункцииОбработчика, СоответствиеОбъектов, П1 = Null, П2 = Null, П3 = Null, П4 = Null, П5 = Null, П6 = Null, П7 = Null)
	Прерывание = ПрерываниеВыполнения();
	Для Каждого ФункцияОбработчика Из ФункцииОбработчика Цикл
		Если ЗначениеЗаполнено(ФункцияОбработчика.ИмяМетаданных) Тогда
			МассивОбъектов = СоответствиеОбъектов[ФункцияОбработчика.ИмяМетаданных];
		Иначе
			МассивОбъектов = СоответствиеОбъектов[Адресация__ВсеОбъекты()];
		КонецЕсли;
...
Функция ФункцииОбработчиковОбъектов(ИменаОбработчиков, МассивИменМетаданных) Экспорт
	МассивФункций = Новый Массив;
	Для Каждого ИмяМетаданных Из МассивИменМетаданных Цикл
		ФункцииОбработчикаОбъекта = ФункцииОбработчикаОбъекта(ИменаОбработчиков, ИмяМетаданных, Истина);
		Для Каждого ФункцияОбработчика Из ФункцииОбработчикаОбъекта Цикл
			МассивФункций.Добавить(ФункцияОбработчика);
		КонецЦикла;
	КонецЦикла;
...
Функция ДобавитьФункциюВМассив(МассивФункций, Пара, ВсеМеханизмы, ИмяОбработчика, ИмяМетаданных = Неопределено)  
	...
	Если А1Э_Структуры.ЯвляетсяСтруктурой(ДанныеОбработчика) Тогда
		ЗаполнитьЗначенияСвойств(ФункцияОбработчикаОбъекта, ДанныеОбработчика);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ИмяМетаданных) Тогда
		ФункцияОбработчикаОбъекта.ИмяМетаданных = ИмяМетаданных;
	КонецЕсли;
...
Функция ИмяФункцииОбработчика(Механизм, ИмяОбработчика) Экспорт 
	...
	ИначеЕсли А1Э_Структуры.ЯвляетсяСтруктурой(ДанныеОбработчика) Тогда
		Возврат А1Э_Структуры.ЗначениеСвойства(ДанныеОбработчика, "ИмяФункции",  Механизм.Модуль + "." + Механизм.Префикс + ИмяОбработчика); 
	Иначе
...
Функция ИмяМетаданныхОбъекта(Объект) Экспорт  
	...
	ИмяФормы = ИмяФормыЕслиЭтоФорма(Объект);
	Если ЗначениеЗаполнено(ИмяФормы) Тогда
		Возврат А1Э_Строки.Перед(ИмяФормы, ".Форма.");
	КонецЕсли;
	
	Ссылка = ?(ТипЗнч(Объект) = Тип("Тип"), Новый(Объект), Новый(ТипЗнч(Объект)));
	Код = А1Э_Метаданные.СвязиСсылокИСистемныхКодов().ПоСсылке[Ссылка];
	Возврат А1Э_Метаданные.ПолноеИмяМетаданныхПоКоду(Код);
КонецФункции

Функция ИмяФормыЕслиЭтоФорма(Объект) Экспорт
	Результат = Новый Структура("ИмяФормы");
	ЗаполнитьЗначенияСвойств(Результат, Объект);
	Возврат Результат.ИмяФормы;
КонецФункции
...
#Если НЕ Клиент Тогда
	...
	Функция МассивМеханизмовИзОбщихМодулей()
		...
		Для Каждого ОбщийМодуль Из ОбщиеМодулиСписковМеханизмов() Цикл
			Попытка
				Результат = ОбщийМодуль.Модуль.ДобавитьМеханизмы(МассивМеханизмов);
			Исключение
	...
	Функция РегистрацииМеханизмовИзОбщихМодулей(ПовторноеИспользование = Истина) Экспорт
		...
		Для Каждого ОбщийМодуль Из ОбщиеМодулиСписковМеханизмов() Цикл
			Попытка
				Результат = ОбщийМодуль.Модуль.ДобавитьОбъекты(СписокОбъектов);
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
	...
	Функция ОбщиеМодулиСписковМеханизмов(ПовторноеИспользование = Истина) Экспорт
		...
		Для Каждого ОбщийМодуль Из Метаданные.ОбщиеМодули Цикл
			Если СтрНайти(ОбщийМодуль.Имя, "А1СписокМеханизмов") Тогда
				Модули.Добавить(А1Э_Структуры.Создать(
				"Имя", ОбщийМодуль.Имя,
				"Модуль", Вычислить(ОбщийМодуль.Имя),
				));
			КонецЕсли;
		КонецЦикла;
		Возврат Модули;
	...
	Функция СоздатьРасширениеМеханизмов(Знач Объекты = Неопределено, Знач КаталогСоздания = Неопределено, Знач Контекст = Неопределено) Экспорт
		...
		Для Каждого ОбъектМетаданных Из ОбъектыМетаданных Цикл
			...
			Для Каждого Форма Из ОбъектМетаданных.Формы Цикл
				...
				Если Форма.ТипФормы = Метаданные.СвойстваОбъектов.ТипФормы.Обычная Тогда Продолжить; КонецЕсли;
...
Функция ПередНачаломРаботыСистемы(Отказ) Экспорт 
	...
	//Проверка чтобы не вызывать сервер если ничего не выполняется.
	Если РабочиеМеханизмы("ПередНачаломРаботыСистемыВызовСервера").Количество() <> 0 Тогда
		А1Э_ОбщееСервер.РезультатФункции("А1Э_Механизмы.ВыполнитьМеханизмыОбработчика", "ПередНачаломРаботыСистемыВызовСервера", Отказ, ДанныеСервера);	
	КонецЕсли;
	ВыполнитьМеханизмыОбработчика("ПередНачаломРаботыСистемыПослеВызоваСервера", Отказ, ДанныеСервера);
КонецФункции
...
Функция ПриНачалеРаботыСистемы() Экспорт
	...
	ДанныеСервера = Новый Структура;
	//Проверка чтобы не вызывать сервер если ничего не выполняется.
	Если РабочиеМеханизмы("ПриНачалеРаботыСистемыВызовСервера").Количество() <> 0 Тогда
		А1Э_ОбщееСервер.РезультатФункции("А1Э_Механизмы.ВыполнитьМеханизмыОбработчика", "ПриНачалеРаботыСистемыВызовСервера", ДанныеСервера);	
	КонецЕсли;
    ВыполнитьМеханизмыОбработчика("ПриНачалеРаботыСистемыПослеВызоваСервера", ДанныеСервера);
КонецФункции 
...
// Обработчик события "ПередЗаписью", вызываемый из подключаемого кода механизмов.
//
// Параметры:
//  Объект	 - 	 - 
//  Отказ	 - 	 - 
//  П1		 - 	 - РежимЗаписи (для документов), Замещение (для регистров сведений)
//  П2		 - 	 - РежимПроведения (для документов)
// 
// Возвращаемое значение:
//   - 
//
Функция ПередЗаписью(Объект, Отказ, П1 = Null, П2 = Null) Экспорт
	ВыполнитьМеханизмыОбработчикаОбъекта("ПередЗаписью", Объект, Отказ, П1, П2);
КонецФункции

// Обработчик события "ПриЗаписи", вызываемый из подключаемого кода механизмов.
//
// Параметры:
//  Объект	 - 	 - 
//  Отказ	 - 	 - 
//  П1		 - 	 - Замещение (для регистров сведений). 
// 
// Возвращаемое значение:
//   - 
//
Функция ПриЗаписи(Объект, Отказ, П1 = Null) Экспорт
	ВыполнитьМеханизмыОбработчикаОбъекта("ПриЗаписи", Объект, Отказ, П1);
КонецФункции
...
Функция ОбработкаФормированияПоВерсииИсторииДанных(Объект, П1, П2, П3, П4, П5 = Null) Экспорт 
	ВыполнитьМеханизмыОбработчикаОбъекта("ОбработкаФормированияПоВерсииИсторииДанных", Объект, П1, П2, П3, П4, П5);
КонецФункции
