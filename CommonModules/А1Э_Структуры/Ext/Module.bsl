...
Функция СкопироватьСвойства(Приемник, Источник, Замещать = Ложь, КопироватьНезаполненные = Истина) Экспорт
	...
	Для Каждого Пара Из Источник Цикл
		Если Приемник.Свойство(Пара.Ключ) И Замещать <> Истина Тогда Продолжить; КонецЕсли;
		Если НЕ КопироватьНезаполненные И НЕ А1Э_СтандартныеТипы.Заполнено(Пара.Значение) Тогда	Продолжить;	КонецЕсли;
