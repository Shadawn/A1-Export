...
Функция МассивОписаний(ПовторноеИспользование = Истина) Экспорт
	...
	КонецЕсли;
	Если НЕ А1Э_Метаданные.ОбъектСуществует("ОбщиеМодули", "А1_ПериодическиеРеквизиты") Тогда
...
#Если НЕ Клиент Тогда
	...
	Функция ПриСозданииНаСервере(Форма, ГруппаФормыДляПериодическихРеквизитов) Экспорт 
		...
		Для Каждого Описание Из МассивОписанийПериодическихРеквизитов Цикл
			...
			"А1_ОбработчикКоманд", Ложь, "Записать (" + Описание.Заголовок + ")", РодительКоманд); 
			А1Э_Формы.ДобавитьОписаниеКомандыИКнопки(МассивОписанийОбъектовФормы, ИмяКомандыИсторииИзменения(Описание.Имя),
			"А1_ОбработчикКоманд", Истина, "История изменений (" + Описание.Заголовок + ")", РодительКоманд);
			Строка = ТаблицаРеквизитов.Добавить();
			ЗаполнитьЗначенияСвойств(Строка, Описание);
		КонецЦикла;
		А1Э_Формы.ДобавитьРеквизитыИЭлементы(Форма, МассивОписанийОбъектовФормы);
...
#Если Клиент Тогда
	...
	Функция КомандаИсторииИзменения(Форма, Команда, ДанныеКоманды) Экспорт
		Если НЕ ЗначениеЗаполнено(Форма.Объект.Ссылка) Тогда
		...
		ОткрытьФорму("РегистрСведений.А1_ПериодическиеРеквизиты.ФормаСписка", Параметры,,,,,,РежимОткрытияОкнаФормы.Независимый);  
	КонецФункции
	
	Функция КомандаЗаписиЗначения(Форма, Команда, ДанныеКоманды) Экспорт
		Если НЕ ЗначениеЗаполнено(Форма.Объект.Ссылка) Тогда
...
Функция ИмяРеквизитаФормы(Имя)
	Возврат "ПериодическийРеквизит__" + Имя;	
КонецФункции

Функция ИмяРеквизитаИсходногоЗначенияФормы(Имя)
	Возврат "ПериодическийРеквизитИсходноеЗначение__" + Имя;
КонецФункции

Функция ИмяКомандыИсторииИзменения(Имя)
	Возврат "ПериодическийРеквизитИсторияИзменений__" + Имя; 	
КонецФункции 

Функция ИмяГруппыКоманд(Имя)
	Возврат "ПериодическийРеквизитГруппаКоманд__" + Имя;	
КонецФункции

Функция ИмяКомандыЗаписиЗначения(Имя)
	Возврат "ПериодическийРеквизитЗаписатьЗначение__" + Имя;	
КонецФункции 
